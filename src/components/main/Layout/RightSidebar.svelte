	...
	{#if visible}
		<div class="fixed right-0 top-0 h-full md:w-[580px] bg-white shadow-lg z-50">
			<div class="flex flex-col justify-between h-full">
				<div
					class="flex justify-between items-center mb-4 border-b-2 border-[#A3AED0] h-[92px] p-5"
				>
					<div>
				...
				<div
					class="flex justify-end items-center gap-3 font-satoshi-regular border-t-2 border-[#A3AED0] p-5"
				>
					<button
						class="border-gradient text-[#60269E] h-[56px] w-[185px] p-4 bg-white"
						onclick={close}>Go Back</button
					>
					<button class="h-[56px] w-[185px] rounded-[8px] p-4 bg-gradient text-white mr-1"
						>Clear All {title === 'Add Text Content' ? 'Chat' : 'History'}</button
					>
				</div>
			</div>
		</div>
